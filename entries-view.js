...
export function entriesTableHtml(data,year,today=currentMonth()){
 const months=entryMonths(data,year,today),amounts=new Map(data.entries.map(entry=>[entry.sourceId+'|'+entry.month,entry.amount]));
 let rows='';
 ...
 rows+=`<tr class="table-total"><td>Итого, ₽</td>${months.map(month=>{const entries=data.entries.filter(entry=>entry.month===month);return `<td>${entries.length?escapeHtml(money(entries.reduce((total,entry)=>total+entry.amount,0)).replace(/\s*₽/,'')):'—'}</td>`;}).join('')}</tr>`;
...
export function monthFieldsHtml(data,month){
 ...
 for(const active of [true,false]){
  ...
  html+=sources.map(source=>{
   const entry=data.entries.find(item=>item.month===month&&item.sourceId===source.id),value=entry?(entry.amount/100).toString().replace('.',','):'';
