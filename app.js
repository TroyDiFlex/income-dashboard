...
function openSource(id){if(busy)return;const s=data.sources.find(s=>s.id===id);$('trash-source').hidden=!s;$('source-id').value=s?.id||'';$('source-name').value=s?.name||'';$('source-active').checked=s?.active??true;$('source-title').textContent=s?'Настроить источник':'Новый источник';$('source-error').textContent='';sourceColor=s?.color||COLORS[data.sources.length%COLORS.length];renderColors();$('source-dialog').showModal();}
