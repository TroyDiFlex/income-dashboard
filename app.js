...
function openEdit(sourceId,month=currentMonth()){if(view==='entries'&&entryMode==='month'&&!discardAllowed($('month-form')))return;if(!data.sources.length){openSource();return;}dirtyForms.delete($('edit-form'));$('edit-error').textContent='';$('edit-title').textContent=sourceId?'Изменить доход':'Добавить доход';$('edit-source').innerHTML=sortSources(data.sources).map(s=>`<option value="${esc(s.id)}">${esc(s.name)}${s.active?'':' · неактивный'}</option>`).join('');if(sourceId)$('edit-source').value=sourceId;$('edit-month').value=month;fillEditAmount();$('edit-dialog').showModal();setTimeout(()=>$('edit-amount').focus(),0);}
...
function renderMonth(){const month=$('entry-month').value||currentMonth();renderedEntryMonth=month;dirtyForms.delete($('month-form'));$('entry-month').value=month;let html='';for(const active of [true,false]){const sources=sortSources(data.sources).filter(s=>s.active===active);if(!sources.length)continue;html+=`<div class="month-fields-label">${active?'АКТИВНЫЕ':'НЕАКТИВНЫЕ'}</div>`;html+=sources.map(s=>{const entry=data.entries.find(e=>e.month===month&&e.sourceId===s.id);return `<div class="month-row"><button type="button" class="month-source ${active?'':'inactive'}" data-source-edit="${esc(s.id)}" aria-label="Настроить источник ${esc(s.name)}"><i class="source-dot" style="background:${s.color}"></i><span>${esc(s.name)}</span>${ico('dots')}</button><input aria-label="${esc(s.name)}, сумма за ${monthLabel(month)}" id="month-${esc(s.id)}" data-month-source="${esc(s.id)}" type="text" inputmode="decimal" placeholder="Нет записи" value="${entry?(entry.amount/100).toString().replace('.',','):''}" autocomplete="off"></div>`;}).join('');}$('month-fields').innerHTML=html||'<div class="empty-state">Добавьте источник дохода, чтобы начать.</div>';$('month-error').textContent='';monthTotal();}
